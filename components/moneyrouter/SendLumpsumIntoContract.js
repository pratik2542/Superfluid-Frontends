...
function SendLumpsumIntoContract({ setDirectContract, setSendLumpsum, theme }) {
  ...
  const sendLumpsumTokens = async () => {
    ...
    try {
      ...
      if (ethereum) {
        ...
        // get allowence data from ERC20 contract
        const allowence = await daix.allowance({
          owner: address,
          spender: moenyRouterContractAddress,
          providerOrSigner: signer,
        });
        const tokenAmount = document.getElementById("tokenAmount").value;
        const tokenAmountInWei = tokenAmount * 10 ** 18;

        // check for enough allowence
        if (tokenAmountInWei > allowence) {
          const tokensShouldApprove = tokenAmountInWei - allowence;
          const moneyRouterApproval = daix.approve({
            receiver: moenyRouterContractAddress,
            amount: ethers.utils.parseEther(String(tokensShouldApprove)),
          });
  ...
  return (
    <div className="main-container">
      <div className="box-container">
        <div className="set-permission-title flex flex-row justify-between">
          <span className="permission-title">Send Lumpsum into Contract</span>
          <span
            className="go-back"
            onClick={() => {
    ...
    /> */}
        ...
        <div className="set-permission-button">
          <button onClick={() => sendLumpsumTokens()}>Send Token</button>
        </div>
      </div>
      {loading ? (
        <TransactionWaiting
          ...
          streamContent={"You are sending a token into contract."}
        />
      ) : null}
    </div>
  );
...
export default SendLumpsumIntoContract;

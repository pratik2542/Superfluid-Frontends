...
function CreateStreamFromContract({ setFlowFC, setCreateStreamFC, theme }) {
  ...
  const sendStreamFromContract = async () => {
    setLoading(true);
    try {
      ...
      if (ethereum) {
        ...
        // get receiver address and flow rate
        const flowRate = document.getElementById("flowRate").value;
        const receiverWalletAddress = document.getElementById(
          "receiverWalletAddress"
        ).value;

        // call contract function to create flow from contract
        await moneyRouter
          .connect(signer)
          .createFlowFromContract(daix.address, receiverWalletAddress, flowRate)
          .then(async function (tx) {
            setTransactionCompleted(tx.hash);
            await tx.wait();
            sendNotification(flowRate, tx.hash, receiverWalletAddress);
            setLoading(false);
            console.log(`
  ...
  const sendNotification = async (rate, txhash, rcvAddress) => {
    try {
      const apiResponse = await PushAPI.payloads.sendNotification({
        ...
        notification: {
          ...
          body: `Stream has been started into your account from the Superfluid Money Router Contract. Flow rate - ${rate} wei/sec`,
        },
        payload: {
          title: `Stream Updates`,
          body: `Stream has been started into your account from the Superfluid Money Router Contract. Flow rate - ${rate} wei/sec`,
        ...
        recipients: `eip155:5:${rcvAddress}`, // recipient address
    ...
    <div className="main-container">
      <div className="box-container">
        <div className="set-permission-title flex flex-row justify-between">
          <span className="permission-title">Create Stream From Contract</span>
          <span
            className="go-back"
            onClick={() => {
              setCreateStreamFC(false);
              setFlowFC(true);
            }}
          >
            <svg
...
/> */}
        ...
        <div className="set-permission-button">
          <button onClick={() => sendStreamFromContract()}>Send Stream</button>
        </div>
      </div>
      {loading ? (
        <TransactionWaiting
          ...
          streamContent={"You are creating a stream from contract."}
        />
      ) : null}
    </div>
  );
...
export default CreateStreamFromContract;
